...
function About() {
  ...
  return (
    <Grid container style={{ textAlign: "center" }}>
      ...
      <Grid item xs={12}>
        <h2 className="name_title">{state.data?.name.toUpperCase()}</h2>
        <br />
        <h4 className="description">{state.data?.profession}</h4>
      </Grid>
      <Grid item xs={12}>
        <MailIcon className="media_icon" onClick={() => mediaClick(1)} />
        <TwitterIcon className="media_icon" onClick={() => mediaClick(2)} />
